...
\def\LOGO{%
    \begin{picture}(0,0)\unitlength=2cm
        \put (0,-0.55) {\includegraphics[width=2cm]{logo45.png}}
       % \put (0.045,-0.6) {\includegraphics[width=3cm]{logo.png}}
    \end{picture}
}

\newcommand{\plannerof}[2]{%
    \def\disciplina{\classtitle{#2#1}}
    \def\gradogrupo{\leveltitle{#1}}
    \def\schoolinfounit{\RaggedRight%
        {\hspace{-5pt}\color{rdsColor}\LARGE\shadowtext{Escuela Rafael Díaz Serdán}}\\[-0.2em]
        {\color{gray!40!black}\sffamily\small 30PES0329R \hspace{9em} turno matutino  }\\[-0.8em]
        {\color{green!20!black}\Large\sffamily Bienvenida al ciclo escolar}\\
    ...
    % }
    \def\cicloescolar{%
        \tcbox[colback=rdsColor!10!white,colframe=rdsColor,size=fbox,on line]{2026-2027}
    }
    \JSONParseFromFile{\weekdata}{../resources/json/2627/#2#1_general.json}
